	...
	while (TRUE)
	{
		if (FindFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
		{
			if (FindFileData.cFileName[0] != '.')
			{
				...
				HTREEITEM tmpParent = tree->InsertItem(FindFileData.cFileName, 0, 0, parent);

				loopFilePath(szFile, tmpParent, tree);
			}
		}
		else
		{
...
{
	HTREEITEM hRoot = m_wndFileView.InsertItem(_T("Resources"), 0, 0);
	m_wndFileView.SetItemState(hRoot, TVIS_BOLD, TVIS_BOLD);

	std::string path = MyUtils::getCheckPath();
	printf("%s", path.c_str());

	WCHAR wpath[MAX_PATH] = { 0 };
	MyUtils::charTowchar(path.c_str(), wpath, sizeof(WCHAR)*path.size());
	loopFilePath(wpath, hRoot, &m_wndFileView);
